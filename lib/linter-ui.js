    ...
    for (const buffer of this.getBuffers()) {
      ...
      if (removedMessages?.length) {
        for (const message of removedMessages) {
          const markers = markerMap.get(message.key);
          if (markers) {
            if (markers.text) markers.text.destroy();
            if (markers.high) markers.high.destroy();
      ...
      if (addedMessages?.length) {
        for (const message of addedMessages) {
          ...
          // If marker starts at line end, start from line beginning instead
          if (range.start.column >= lineLength) {
            range = range.copy();
          ...
          markerMap.set(message.key, { text: textMarker });
        }
      }
