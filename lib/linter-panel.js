...
class LinterPanel {
  constructor(pkg) {
    ...
    // Register context menu commands and entries
  ...
  _getMessages() {
    ...
    return buffer.linterUI.messages;
  }
  ...
  _onRowClick(event) {
    ...
    if (this.viewMode === "project") {
      ...
      // In file mode, navigate within current editor
      if (!this.editor) return;
    ...
    if (this.sortMethod === "severity") {
      ...
      sortedMessages = [...messages].sort((a, b) => {
        if (byFile) {
          ...
          if (fileA < fileB) return this.sortDirection === "asc" ? -1 : 1;
          if (fileA > fileB) return this.sortDirection === "asc" ? 1 : -1;
        }
  ...
  _copyDetails() {
    ...
    if (!message) return;
    atom.clipboard.write(JSON.stringify(message, (k, v) =>
      k === 'key' || k === 'version' || k === 'displayRange' ? undefined : v
    , 2));
  }
  ...
  update() {
    // Clear .current before re-render since etch doesn't know about it
    const currentRow = this.element?.querySelector(".linter-row.current");
