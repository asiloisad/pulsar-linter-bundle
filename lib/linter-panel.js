...
class LinterPanel {
  constructor(pkg) {
    ...
    this.sortMethod = atom.config.get("linter-bundle.defaultSortMethod") || "severity";
    ...
    this._disposables = atom.commands.add(this.element, {
      ...
      "linter-bundle:copy-details": () => this._copyDetails(),
      "core:move-up": (e) => { e.stopPropagation(); this._moveFocusUp(); },
      "core:move-down": (e) => { e.stopPropagation(); this._moveFocusDown(); },
      "core:confirm": (e) => { e.stopPropagation(); this._confirmFocused(); },
      "core:cancel": (e) => { e.stopPropagation(); this._cancelFocus(); },
    });
    atom.contextMenu.add({
      ".linter-wrapper .linter-row": [
        { label: "Copy Description", command: "linter-bundle:copy-description" },
        { label: "Copy Details", command: "linter-bundle:copy-details" },
      ],
    });
  }
  ...
  _moveFocusDown() {
    ...
    this._setFocusedRow(this._focusedRowIndex < 0 ? 0 : Math.min(this._focusedRowIndex + 1, count - 1));
  }
  ...
  _moveFocusUp() {
    ...
    if (!tbody || !tbody.children.length) return;
    const count = tbody.children.length;
    this._setFocusedRow(this._focusedRowIndex < 0 ? count - 1 : Math.max(this._focusedRowIndex - 1, 0));
  }
  ...
  scrollToFocused() {
    ...
    const rowTop =
      focusedRow.getBoundingClientRect().top -
