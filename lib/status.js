class StatusPanel {
  constructor(pkg) {
    ...
    this.tooltipDisposable = atom.tooltips.add(this.element, {
      title: '<div style="text-align: left; line-height: 1.2em;">Click to toggle linter<br />Middle-click to toggle file/project view<br />Ctrl+middle-click to clear all<br />Right-click to go next<br />Ctrl+right-click to go previous</div>',
      html: true,
    });
  ...
  update() {
    ...
    this.element.classList.toggle("linter-status-hidden", !this.statusMode && ecount + wcount + icount === 0);
  }
  ...
  onmouseup(e) {
    if (e.which === 2 && e.ctrlKey) {
      ...
      // left click
      atom.commands.dispatch(atom.views.getView(atom.workspace), "linter-bundle:toggle-linter");
    } else if (e.which === 2) {
      // middle click
